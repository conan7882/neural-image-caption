...
def Inception_conv_embedding(inputs, pretrained_dict=None, name='Inception_conv'):
    inputs = sub_rgb2bgr_mean(inputs)
    layer_dict = {}
    with tf.variable_scope('Inception_conv_embedding'):
        with tf.variable_scope('conv_layers'):
            conv_out = inception_conv_layers(
                layer_dict=layer_dict, inputs=inputs,
                pretrained_dict=pretrained_dict,
                is_training=False, trainable=False)
        with tf.variable_scope('inception_layers'):
            inception_out = inception_layers(
                layer_dict=layer_dict, inputs=conv_out,
                pretrained_dict=pretrained_dict,
                is_training=False, trainable=False)
