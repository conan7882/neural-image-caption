...
def rnn_layer(inputs, hidden_size_list, init_state=None,
              is_training=True, keep_prob=1.0, name='rnn_layer'):
    def _make_cell(n_hidden):
    ...
    with tf.variable_scope(name):
        cell = tf.contrib.rnn.MultiRNNCell(
            [_make_cell(hidden_size)
             for hidden_size in hidden_size_list], state_is_tuple=True)
...
# def create_rnn_layer(hidden_size_list, init_state=None,
#                      is_training=True, keep_prob=1.0, name='rnn_layer'):
